...
// fp32 storage roughly doubles the tile kernel's LDS; shrink nbatch_K until it fits one block.
static constexpr __device__ int ggml_cuda_fattn_tile_get_nbatch_K_f32(
    ...
    // 0 means fp32 won't fit this shape, so the caller keeps fp16.
    const int step = 8;
...
#ifdef GGML_USE_HIP
    // AMD non-GQA fallback: honor GGML_PREC_F32 here, or fp16 corrupts the result.
